        ...
        for (; allowCapacity>0; ){
            ...
            while (!toBeVisit.empty()){ //如果是从while的条件break的，就是没找到合适的路径，但是找到了所有source可达点的最短路径。
                ...
                for (int j = 1; j <= vertexCnt; ++j) { //遍历连接的所有边。
                    if (!isVisited[j] && CapacityFromIToJ(residualGraph, top, j)>=allowCapacity){
                        ...
                        parent[j] = top; //上面那个isVisited很重要，保证了parent不会被人乱改。parent始终是记录最短路径的。
                        toBeVisit.push(j);
                    }
                }
            }
            if (!cannotFindPath){
                ...
                for (size_t it=sink; it!=source; it = parent[it]){
                    assert(it!=0);
                    bottleNeck = std::min(bottleNeck, CapacityFromIToJ(residualGraph, parent[it], it));
                }
                for (size_t it=sink; it!=source; it = parent[it]){
                    assert(it!=0);
                    CapacityFromIToJ(residualGraph, parent[it], it)-=bottleNeck;
                    CapacityFromIToJ(residualGraph, it, parent[it])+=bottleNeck;
                }
